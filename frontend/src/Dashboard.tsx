...
function Dashboard() {
   ...
   const handleGoogleLogin = async () => {
    ...
    // Fetch all files with pagination (returns array of files)
    const fetchAllDriveFilesFromGoogle = async (accessToken) => {
        let allFiles = [];
        let nextPageToken = null;
        const pageSize = 1000; // max allowed per page is usually 1000
        ...
        try {
            ...
            console.error("fetchAllDriveFilesFromGoogle error:", err);
            throw err;
        }
    };
    ...
    useEffect(() => {
        const startFullDriveSync = async () => {
            try {
                ...
                // 2) set to local state (if needed for UI)
                setGoogleDriveFiles(allFiles);
                localStorage.setItem("google_drive_files", JSON.stringify(allFiles));

                // 3) (optional) build tree
                // const tree = buildDriveTree(allFiles);
                // localStorage.setItem("google_drive_tree", JSON.stringify(tree));

                // 4) send to backend in batches
                await sendFilesToBackendInBatches(accessToken, allFiles, 500);

                // 5) mark as done (prevents re-run)
                localStorage.setItem("drive_fetch_needed", "false");
        ...
        // call it
        startFullDriveSync();
    }, []); // empty deps - runs once on Dashboard mount

    const sendFilesToBackendInBatches = async (accessToken, files, batchSize = 500) => {
        const format = (file) => ({
            id: file.id,
            name: file.name,
            mimeType: file.mimeType,
        ...
        for (let i = 0; i < files.length; i += batchSize) {
            const batch = files.slice(i, i + batchSize).map(format);

            const res = await fetch("https://ecommerce-web-4pmx.onrender.com/api/store-google-drive-files", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${accessToken}`, // keep for backend verification if needed
                },
            ...
            if (!res.ok) {
                const text = await res.text();
                throw new Error(`Backend batch upload failed: ${res.status} ${text}`);
            }

            // optional: read response to show progress
            await res.json();
        }
    };
    ...
    const buildDriveTree = (files) => {
        ...
        files.forEach(f => {
            if (f.parents && f.parents.length > 0) {
                const parent = map[f.parents[0]];
                if (parent) parent.children.push(map[f.id]);
